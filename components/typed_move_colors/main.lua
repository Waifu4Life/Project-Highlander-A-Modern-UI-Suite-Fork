...
return function(mod)
  ...
  local ballOk, ballErr = pcall(function()
    return mod:load("colored_pokeballs.lua")(mod)
  end)
  if not ballOk then
    mod.log:error("colored pokeballs failed: %s", tostring(ballErr))
  end
  local moveOk, moveErr = pcall(function()
    return mod:load("colored_pokemoves.lua")(mod)
  end)
  if not moveOk then
    mod.log:error("colored pokemoves failed: %s", tostring(moveErr))
  end
