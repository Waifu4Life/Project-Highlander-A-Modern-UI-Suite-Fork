...
return function(mod, compatibility)
  ...
  local function categoryFor(game, id)
    ...
    if KEY_ITEMS[id] or def.keyItem or def.key then return "key" end
    ...
    for _, id in ipairs(orderedIds(menu, store)) do
      if included(menu, id)
        ...
        rows[#rows + 1] = {
          ...
          right = (pocketKey == "key" or KEY_ITEMS[id] or (def and (def.keyItem or def.key)))
            and "" or ("x" .. tostring(store[id] or 0)),
        }
      end
    end
    return rows
