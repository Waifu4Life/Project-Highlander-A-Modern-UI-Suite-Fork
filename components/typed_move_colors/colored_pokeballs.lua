...
-- per-ball toss palette, wide party rows, Pokémon Center machine.
...
return function(mod)
  ...
  local HEAL = { { 248, 248, 248 }, { 232, 80, 40 }, { 24, 24, 24 } }
  local function seedHealImage(state)
    if not enabled() or state.healMachineImg ~= nil then return end
  ...
  if not Overworld._suiteBallColorOriginalEnter then
    Overworld._suiteBallColorOriginalEnter = Overworld.enter
  end
  local originalEnter = Overworld._suiteBallColorOriginalEnter
