  ...
  local function buildItems(game, iconsOnly)
    ...
    if not iconsOnly then
      items = {
        ...
        {
        id = mod.id .. ":theme_scope",
        label = "COLOUR SCOPE",
        right = config.scopeLabel and config.scopeLabel() or "GAME",
        kind = "scope",
        },
        {
        id = mod.id .. ":position",
  ...
  function Settings.newMenu(game, iconsOnly)
    ...
    local function choose(item, activeMenu)
      ...
      elseif item.kind == "theme" then
        ...
        persist()
        refresh(item.id)
      elseif item.kind == "scope" then
        if config.stepScope then config.stepScope(game, 1) end
        persist()
        refresh(item.id)
    ...
    menu.update = function(self, dt)
      ...
      local input = self.game and self.game.input
      if item and (item.kind == "theme" or item.kind == "scope"
          or item.kind == "position" or item.kind == "clock") and input then
        local step = item.kind == "theme" and config.stepTheme
          or item.kind == "scope" and config.stepScope
