    ...
    if type(nativeUpdateMoveDetail) == "function" then
      ...
      menu.updateMoveDetail = function(self, input)
        ...
        if input:wasPressed("select") then
          local index = math.max(1, math.min(4, tonumber(self.moveIndex) or 1))
          local held = tonumber(self.modernMoveSwap)
          if not held then
            if moves[index] then self.modernMoveSwap = index end
          else
            if held ~= index then
              moves[held], moves[index] = moves[index], moves[held]
              local packed = {}
              for i = 1, 4 do
                if moves[i] then packed[#packed + 1] = moves[i] end
              end
              for i = 1, 4 do moves[i] = packed[i] end
            end
            self.modernMoveSwap = nil
          end
          return
        elseif self.modernMoveSwap and input:wasPressed("b") then
          self.modernMoveSwap = nil
          return
        end
        if renderedWidth >= 196 then
          for _, direction in ipairs({ "left", "right", "up", "down" }) do
            if input:wasPressed(direction) then
              local list = self.moveList and self:moveList()
                or (self.mon and self.mon.moves) or {}
              self.moveIndex = moveGridIndex(self.moveIndex, #list, direction)
                or self.moveIndex
              return
            end
          end
