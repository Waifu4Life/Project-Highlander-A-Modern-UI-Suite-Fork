...
return function(mod, compatibility)
  ...
  local function applyDexFilter(screen, letter, typeId)
    screen.modernDexShinyOnly = false
  ...
  local function drawHeader(screen, layout)
    ...
    drawText(screen.modernDexShinyOnly and "SHINY DEX" or "POKéDEX", 5, 4, 64, WHITE)
  ...
  local function drawList(screen, layout, regions)
    ...
    for visible = 1, layout.rows do
      ...
      if row.owned or shinyCaught(screen.game, row.def and row.def.id) then
        drawCollectionMark(screen.game, row.def and row.def.id, rect.x + rect.w - 10, rect.y + 9)
      end
    end
  ...
  local function drawPreview(screen, layout, regions)
    ...
    if row.owned or shinyCaught(screen.game, row.def and row.def.id) then
      drawCollectionMark(screen.game, row.def and row.def.id, rect.x + rect.w - 11, infoY + 4)
    end
